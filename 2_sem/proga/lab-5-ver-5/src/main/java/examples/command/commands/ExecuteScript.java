...
public class ExecuteScript extends Command {
    ...
    public void execute(String args) throws IllegalArgumentException {
        ...
        if (commandManager.isScriptRecursionExecuted()) {
            console.printError("Рекурсия скрипта уже выполнялась");
            return;
        }
        commandManager.setScriptRecursionExecuted(true);// выполнение команды execute_script
        commandManager.setScriptRecursionExecuted(false);

        try {
            Printable.setFileMode(true);
